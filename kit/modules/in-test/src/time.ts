 ...
 *   spy,
 * } from "@in/test/mock";
 * import { FakeTime } from "@in/test/time";
 *
 ...
 * @example Usage
 * ```ts
 * import { FakeTime, TimeError } from "@in/test/time";
...
function FakeTimeNow() {
  return time?.now ?? _internalsFT.Date.now();
}
...
const FakeDate = new Proxy(Date, {
  construct(_target, args) {
    ...
    return new _internalsFT.Date(...args);
  },
  apply(_target, _thisArg, _args) {
    return new _internalsFT.Date(FakeTimeNow()).toString();
  },
...
function restoreGlobals() {
  globalThis.Date = _internalsFT.Date;
  globalThis.setTimeout = _internalsFT.setTimeout;
  globalThis.clearTimeout = _internalsFT.clearTimeout;
  globalThis.setInterval = _internalsFT.setInterval;
  globalThis.clearInterval = _internalsFT.clearInterval;
  AbortSignal.timeout = _internalsFT.AbortSignalTimeout;
}
...
export class FakeTime {
  ...
  constructor(
    ...
    initializedAt = _internalsFT.Date.now();
    startedAt =
    ...
    timerId = timerIdGen();
    dueNodes = new Map();
    dueTree = new RedBlackTree((a: DueNode, b: DueNode) =>
      ascend(a.due, b.due)
    );
   ...
   * ```ts
   * import { FakeTime } from "@in/test/time";
   * import { assert } from "@in/test/assert";
   *
   * const fakeTime = new FakeTime(15_000);
   *
   * let called = false;
   ...
   * ```ts
   * import { FakeTime } from "@in/test/time";
   * import { assert, assertEquals } from "@in/test/assert";
   *
   * const fakeTime = new FakeTime(15_000);
   *
   * let called = false;
