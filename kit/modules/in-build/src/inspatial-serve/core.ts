...
export class InSpatialServe {
  ...
  private async scanExtensionsAndGenerateTypes() {
    ...
    try {
      ...
      const extensionTriggers: string[] = [];

      try {
        const renderContent = await InZero.readTextFile(renderPath);

        // Extract extension imports and analyze them
        // For now, we'll manually check for known extensions
        // In production, this would use AST parsing

        // Check for @in-cloud
        if (
          renderContent.includes("@in/cloud") ||
          renderContent.includes("InCloud")
        ) {
          // Add cloud triggers
          extensionTriggers.push(
            `  cloudStatus: { type: "cloudStatus"; status: string };`
          );
          extensionTriggers.push(`  cloudReconnected: void;`);
          extensionTriggers.push(
        ...
        const triggerTypesContent = `// Auto-generated by InSpatial Serve - DO NOT EDIT
        ...
        /**
         * Extension trigger types collected from all registered extensions
         */
        export interface ExtensionTriggers {
        ${extensionTriggers.join("\n")}
        }

        /**
         * Combined trigger registry including core and extension triggers
         */
        export type AllTriggers = InUniversalTriggerPropsType | keyof ExtensionTriggers;

        /**
         * Mapped type for JSX on: props
         */
        export type TriggerOnProps = {
          [K in AllTriggers as \`on:\${K}\`]?: K extends keyof ExtensionTriggers
            ? ((payload: ExtensionTriggers[K]) => any) | any
            : ((payload: any) => any) | any;
        };
        `;
        ...
        await InZero.writeTextFile(
          "./dist/trigger-types.generated.d.ts",
          triggerTypesContent
        );
        console.log(
          "✅ Generated trigger types at dist/trigger-types.generated.d.ts"
        );
      } catch (e) {
