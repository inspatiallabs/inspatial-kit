import { describe, expect, it, mockFn } from "@in/test";
import { TestTree } from "../../test-utils/test-tree.ts";
import { asyncDataLoaderFeature } from "./feature.ts";
...
describe("core-feature/selections", () => {
  ...
  describe("data invalidation", () => {
    const getItem = mockFn(async (id) => id);
    const getChildren = mockFn(async (id) => [
      `${id}1`,
      `${id}2`,
      `${id}3`,
      `${id}4`,
  ...
  describe("getChildrenWithData", () => {
    const getChildrenWithData = mockFn(async (id) => [
      { id: `${id}1`, data: `${id}1-data` },
      { id: `${id}2`, data: `${id}2-data` },
    ]);
    const getItem = mockFn();
