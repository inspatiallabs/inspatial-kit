import type { FeatureImplementation, TreeInstance } from "../../types/core.ts";
import { makeStateUpdater } from "../../utils.ts";
import { CheckedState } from "./type.ts";
...
const getAllDescendants = async <T>(
  ...
  const descendants = (
    await Promise.all(
      childrenIds.map((child) => getAllDescendants(tree, child, includeFolders))
    )
  ).flat();
  return includeFolders ? [itemId, ...descendants] : descendants;
};
