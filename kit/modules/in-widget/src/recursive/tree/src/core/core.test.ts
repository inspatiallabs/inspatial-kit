import { describe, expect, it, spy } from "@in/test";
import { TestTree } from "../test-utils/test-tree.ts";
import { buildStaticInstance } from "./build-static-instance.ts";

declare module "@in/widget/recursive/tree/types/core.ts" {
...
const handler1 = spy(({ prev }, ...params) => prev?.(...params));
const handler2 = spy(({ prev }, ...params) => prev?.(...params));
  ...
  factory.forSuits((tree) => {
    describe("rebuilds item instance", () => {
      it("rebuilds when explicitly invoked", async () => {
        const instanceBuilder = spy().mockImplementation(buildStaticInstance);
