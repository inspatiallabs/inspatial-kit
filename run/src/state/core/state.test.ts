...
Deno.test("State V2 Tests", async (t) => {
  ...
  await t.step(
    "createAction works with state properties using tuple syntax",
    () => {
  ...
  await t.step(
    "createAction creates multiple triggers using batch syntax",
    () => {
  ...
  await t.step("trigger throttling works", async () => {
    const state = createState({ count: 0 });
    const increment = createAction(state.count, (c) => c + 1, { throttle: 50 });
  ...
  await t.step("trigger debouncing works", async () => {
    const state = createState({ count: 0 });
    const increment = createAction(state.count, (c) => c + 1, { debounce: 50 });
