...
## 🔍 InWidget (🟢 Stable)
InWidget is a universal graphical user interface and component library for web, mobile, xr and games.
...
```bash
deno install jsr:@in/widget
```

##

```bash
npx jsr add @in/widget
```

##

```bash
yarn dlx jsr add @in/widget
```

##

```bash
pnpm dlx jsr add @in/widget
```

##

```bash
bunx jsr add @in/widget
```

##

```bash
vlt install jsr:@in/widget
```
---
...
We welcome contributions to InWidget! Please read our [Contributing Guidelines](CONTRIBUTING.md) to get started.
...
<div align="center">
  <strong>Ready to build the future of graphical user interfaces?</strong>
  <br>
