import { mockFn, expect, spy, stub } from "@in/test";
import { TestTree } from "./test-tree.ts";
import type { HotkeyName } from "../types/core.ts";
import type { HotkeyConfig } from "../features/hotkeys-core/type.ts";
...
export class TestTreeDo<T> {
  ...
  dragOver(itemId: string, event?: DragEvent) {
    const e = event ?? TestTree.dragEvent();
    (e.preventDefault as any).mockClear();
