...
/**
 * CI/CD Environment Utilities
 * Single source of truth for CI detection and metadata
 */
interface CICDProps {
  isCI: boolean;
  provider: string;
  ...
  isCI(): boolean {
    return Boolean(
      env.get("CI") ||
        ...
        env.get("CODEBUILD_BUILD_ID")
    );
  }

  getCICDProvider(): string {
    if (env.get("GITHUB_ACTIONS")) return "GitHub Actions";
    if (env.get("GITLAB_CI")) return "GitLab CI";
    if (env.get("JENKINS_URL")) return "Jenkins";
    if (env.get("TRAVIS")) return "Travis CI";
    if (env.get("CIRCLECI")) return "CircleCI";
    if (env.get("APPVEYOR")) return "AppVeyor";
    if (env.get("AZURE_HTTP_USER_AGENT")) return "Azure DevOps";
    if (env.get("CODEBUILD_BUILD_ID")) return "AWS CodeBuild";
    if (env.get("BUILDKITE")) return "Buildkite";
    if (env.get("DRONE")) return "Drone";
    if (env.get("TEAMCITY_VERSION")) return "TeamCity";
    if (this.isCI()) return "Unknown CI";
    return "Not CI";
  }

  isCICDProvider(provider: string): boolean {
    const normalizedProvider = provider.toLowerCase().replace(/[\s-_]/g, "");
    const currentProvider = this.getCICDProvider()
      .toLowerCase()
      .replace(/[\s-_]/g, "");
    return currentProvider.includes(normalizedProvider);
  }

  getCICDInfo(): CICDProps {
    return {
      isCI: this.isCI(),
      ...
      buildId:
        env.get("BUILD_ID") || env.get("GITHUB_RUN_ID") || env.get("CI_JOB_ID"),
      branch:
        env.get("BRANCH_NAME") ||
      ...
      pullRequest:
        env.get("PULL_REQUEST") ||
        (env.get("GITHUB_EVENT_NAME") === "pull_request"
          ? env.get("GITHUB_EVENT_NUMBER")
          : undefined),
      jobId:
        env.get("JOB_ID") || env.get("GITHUB_JOB") || env.get("CI_JOB_NAME"),
    };
  }
