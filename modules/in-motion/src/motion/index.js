...
export function createMotion() {
  ...
  if (typeof window !== "undefined" && globalThis.InMotion?.initialized()) {
    return globalThis.InMotion;
  }
  ...
  const DATA_InMotion_ATTRIBUTE = "data-inmotion";
  const DATA_InMotion_ID = `${DATA_InMotion_ATTRIBUTE}-id`;
  const DATA_InMotion_SELECTOR = `[${DATA_InMotion_ATTRIBUTE}]`;
  ...
  const ISS_NONE = "none";
  const ISS_INLINE_BLOCK = "inline-block";
  ...
  const genTmpId = () =>
    `__inmotion${Date.now()}_${Math.random().toString(36).slice(2)}`;
  ...
  const captureComputedStyle = (element) => {
    ...
    return {
      ...
      [STYLE_TRANSFORM]: computedStyle[STYLE_TRANSFORM] || ISS_NONE,
      [STYLE_FILTER]: computedStyle[STYLE_FILTER] || ISS_NONE,
      [STYLE_PERSPECTIVE]: computedStyle[STYLE_PERSPECTIVE] || ISS_NONE,
    };
  };
  ...
  function applyStyles(element, styles, clean = false) {
    ...
    const { _offset, _composite, _easing, ...cleanedStyles } = styles;
    ...
    if (instance.destroying == null && !clean) {
      element.__inmotionFragment = 1;
    } else {
      delete element.__inmotionFragment;
      delete element.__inmotionOriginals;
      delete element.__inmotionID;
    }
  }
    ...
    new Promise((resolve) => {
      setTimeout(() => {
        ...
        element
          .getAnimations()
          .filter(
            (animation) => animation.id && animation.id.startsWith("__inmotion")
          )
          .forEach((animation) => {
            animation.cancel();
  ...
  const resetStyle = (data) => {
    ...
    const originalStyle = data.element.__inmotionOriginals?.style;
    if (data.countData) {
      ...
      if (data.targets) {
        data.targets().forEach(([target]) => {
          applyStyles(
            ...
            createKeyframes(
              data.config,
              target.__inmotionOriginals?.style || originalStyle
            )[0]
          );
        });
      }
  ...
  const parseClasses = (classString) => {
    ...
    classString = extractAndSetConfig(
      "count",
      config,
      CONFIG_COUNT,
      classString
        ...
        default:
          switch (firstPart) {
            case "split":
              if (parts[1])
                config[CONFIG_SPLIT] =
                  (config[CONFIG_SPLIT] ?? "") + " " + token.slice(6);
              break;
            case "blur":
  ...
  const createKeyframes = (config, originalStyle) => {
    ...
    const splitByNotItem =
      config[CONFIG_SPLIT] && !config[CONFIG_SPLIT]?.includes("item");
    if (config[CONFIG_LINE])
      return parseTimeline(config[CONFIG_LINE], originalStyle, splitByNotItem);

    const animationType =
      config[CONFIG_ANIMATION] ??
      extractAnimation(instance.config.defaults.animation, 0);
    const direction =
      config[CONFIG_DIRECTION] ??
      extractDirection(instance.config.defaults.direction, 1);
  ...
  const setupSplit = (element, splitBy, strategy) => {
    ...
    words.forEach((word) => {
      ...
      applyStyles(container, {
        [STYLE_DISPLAY]: ISS_INLINE_BLOCK,
        whiteSpace: "nowrap",
      });
  ...
  const setupCount = (element, config, data) => {
    ...
    if (separators.length === 0) {
      ...
      if (afterSep.length <= 3 && afterSep.length > 0 && sep !== " ") {
        ...
        thousandSep = sep;
        value = parseFloat(
          clean.replace(new RegExp(`\\${thousandSep}`, "g"), "")
        );
      }
    } else {
  ...
  const animateCount = (countData, options) => {
    ...
    const { value, decimals, original, span, thousandSep, decimalSep } =
      countData;
    ...
    const formatNumber = (val) => {
      ...
      if (thousandSep && parts[0].length > 3) {
        ...
        parts[0] = reversed.reduce(
          (acc, digit, i) =>
            i > 0 && i % 3 === 0 ? digit + thousandSep + acc : digit + acc,
          ""
        );
      }
      return parts.length > 1 && decimalSep
  ...
  class AnimationController {
    ...
    add(element, config, delay, originalStyle) {
      const duration =
        this.data.config[CONFIG_DURATION] ?? instance.config.defaults.duration;
      const easing =
        this.data.config[CONFIG_EASING] ?? instance.config.defaults.easing;
      const forwards =
        this.data.config[CONFIG_FORWARDS] ?? instance.config.defaults.forwards;
      const loop =
      ...
      if (this.data.config[CONFIG_TEXT]) {
        ...
        keyframes =
          this.data.config[CONFIG_TEXT] === "shimmer"
            ? SHIMMER_KEYFRAMES
            : WEIGHT_KEYFRAMES;
      } else {
    ...
    letsGo() {
      ...
      const duration =
        config[CONFIG_DURATION] ?? instance.config.defaults.duration;
      const originalStyle = element.__inmotionOriginals?.style;
      const splitDelay =
        splitConfig[CONFIG_DELAY] ?? instance.config.defaults.splitDelay;

      let notReadYet = targets?.()?.length || (originalStyle ? 0 : 1);

      if (targets) {
        targets(duration, splitDelay).forEach(([target, delay]) => {
          const targetOriginalStyle =
            target.__inmotionOriginals?.style || originalStyle;
    ...
    prepare() {
      ...
      for (const [animation, info] of this.animations) {
        ...
        try {
          const duration = animation.effect.getTiming().duration;
          if (typeof duration === "number" && duration > 0) {
            const progress = Math.max(0, Math.min(1, (animation.currentTime || 0) / duration));
            if (!isNaN(progress) && isFinite(progress) && Math.abs(progress - (info.lastProgress ?? -1)) >= 0.02) {
              info.lastProgress = progress;
              info.element?.dispatchEvent?.(
                new CustomEvent("inmotion:progress", {
                  bubbles: true,
                  detail: {
                    id: this.data.element?.__inmotionID,
                    ratio: progress,
                    time: animation.currentTime || 0,
                    duration,
                    direction: info.playbackRate >= 0 ? "forward" : "reverse",
                  },
                })
              );
            }
        ...
        toAnimate.forEach((next) => {
          ...
          try {
            next.element?.dispatchEvent?.(
              new CustomEvent("inmotion:start", {
                bubbles: true,
                detail: { id: this.data.element?.__inmotionID },
              })
            );
          } catch {}
          next.animation.play();
  ...
  const animateIfVisible = (data, ratio = null) => {
    ...
    const threshold = Math.max(
      ...
      Math.min(
        1,
        (data.config[CONFIG_THRESHOLD] ?? instance.config.defaults.threshold) /
          100
      )
    );
  ...
  const cleanupElement = (data) =>
    new Promise((resolve) => {
      data.onfinish = () => {
        ...
        if (data.targets) {
          data.targets().forEach(([target]) => {
            if (target.__inmotionOriginals?.style) {
              applyStyles(target, target.__inmotionOriginals.style, true);
            }
          });
        }

        const innerHTML = data.element.__inmotionOriginals?.innerHTML;
        if (
          innerHTML &&
          !splitByItem &&
          (data.config[CONFIG_SPLIT] || data.countData)
        ) {
          data.element.innerHTML = innerHTML;
        }

        if (data.element.__inmotionOriginals?.style) {
          applyStyles(
            data.element,
            data.element.__inmotionOriginals.style,
            true
          );
        }
  ...
  const processElement = (element, elementObserver) => {
    if (!element.__inmotionID) {
      element.__inmotionOriginals = {
        style: captureComputedStyle(element),
        innerHTML: element.innerHTML,
      };
      element.__inmotionID =
        element.getAttribute(DATA_InMotion_ID) ?? genTmpId();
    }

    const classes = element.getAttribute(DATA_InMotion_ATTRIBUTE) || "";

    const existing = instance.elements.get(element.__inmotionID);
    if (existing) {
      if (classes !== existing.configString) {
        instance.elements.delete(element.__inmotionID);
    ...
    if (splitBy) {
      ...
      data.splitConfig = config.map(
        (value, index) => splitOverrides[index] ?? value
      );
    }
    ...
    instance.elements.set(element.__inmotionID, data);
  ...
  const setupObservers = () => {
      ...
      (entries) => {
        for (const entry of entries) {
          const data = instance.elements.get(
            entry.target.__inmotionID ||
              entry.target.getAttribute(DATA_InMotion_ID)
          );
          if (data) {
    ...
    const scanAllDOMs = () => {
      ...
      for (const dom of allDOMs) {
        ...
        const elements = dom.querySelectorAll?.(DATA_InMotion_SELECTOR);
    ...
    const handleObserverEvents = (events) => {
      ...
      const hasUsalFragment = (target) => !!target.__inmotionFragment;
      ...
      for (const item of items) {
        if (item.type === "attributes") {
          ...
          if (
            attrName === DATA_InMotion_ATTRIBUTE ||
            attrName === DATA_InMotion_ID
          ) {
        ...
        if (cancel === null) {
          ...
          if (item.type === "childList") {
            const hasUsalFragmentChild = [
              ...item.addedNodes,
              ...item.removedNodes,
            ].some(hasUsalFragment);
  ...
  const publicAPI = {
    ...
    async destroy() {
      ...
      instance.destroying = Promise.all(
        elements.map((data) => cleanupElement(data))
      ).then(() => {
    ...
    async restart() {
      ...
      if (instance.destroying != null)
        return instance.destroying.then(() => publicAPI.restart());
