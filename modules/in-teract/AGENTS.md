...
- **No 3rd Party Dependecies or NPM Packages**: MUST not install third-party packages or NPM packages. All packages must be installed from JSR and only InSpatial Kit Modules which start with `@in` or `@inspatial` directives.
